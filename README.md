...
| ③ | [7절](#7-금융-데이터-파이프라인-실습-예제) | HDFS EZ + Ozone TDE + **Ranger Iceberg 정책** → 파이프라인 | 10분 |
...
# ②-b Ranger Iceberg-on-Ozone 정책 확인 (플랫폼 팀 등록 후)
bash scripts/security/print_ranger_iceberg_pairs.sh

# ③ HDFS EZ (hdfs_encryption_key) + Ozone bucket (ozone_encryption_key)
...
| 5 | `scripts/security/print_ranger_iceberg_pairs.sh` | 7절 전 — **Cloudera Iceberg–Ozone** Ranger 정책 목록 |
| 6 | `scripts/infrastructure/setup_hdfs_encryption_zone.sh` | 7절 — HDFS Encryption Zone (`hdfs_encryption_key`) |
| 7 | `scripts/infrastructure/setup_ozone_encrypted_bucket.sh` | 7절 — `--bucketkey ozone_encryption_key` |
...
1. 플랫폼 팀에 **`systest` principal** Ranger 정책 요청 — [Cloudera Iceberg–Ozone paired 정책](docs/operations/ranger-iceberg-ozone-pairs.md) (cm_hive Storage Handler + SQL + URL + cm_ozone)
2. `bash scripts/security/print_ranger_iceberg_pairs.sh` — 테이블별 cm_hive/cm_ozone 정책 목록 출력
3. Ranger KMS **`hdfs_encryption_key`** + **`ozone_encryption_key`** ACL — [HDFS Encryption](docs/operations/hdfs-encryption.md) · [Ozone Encryption](docs/operations/ozone-encryption.md)
4. `bash scripts/infrastructure/setup_hdfs_encryption_zone.sh` — **HDFS Encryption Zone**
5. `bash scripts/infrastructure/setup_ozone_encrypted_bucket.sh` — **`--bucketkey ozone_encryption_key`** bucket
6. `bash scripts/security/security_check.sh` — Kerberos + Ranger + KMS + HDFS EZ probe
...
`bash scripts/infrastructure/setup_hdfs_encryption_zone.sh` + `setup_ozone_encrypted_bucket.sh` 로 **HDFS EZ·암호화 bucket**을 준비한 뒤  
...
**허용:** Ranger UI로 HDFS/Ozone/Hive 정책 · Ranger KMS UI로 **`hdfs_encryption_key`** / **`ozone_encryption_key`** ACL · HDFS EZ · bucket `--bucketkey`
...
| **cm_ozone** | volume `dev`, bucket `data` | 인프라 (volume/bucket 생성) |
...
volume `dev`, bucket `data`가 없으면 [7절 HDFS EZ + Ozone 암호화 준비](#최초-실행-전-hdfs-encryption-zone--ozone-암호화-bucket-ranger-kms-tde)를 먼저 진행하세요.
