...
| ③ | [7절](#7-금융-데이터-파이프라인-실습-예제) | Ranger Ozone 정책 확인 → volume/bucket → 소량 파이프라인 | 10분 |
...
> **Kerberos / Ranger:** 실행 전 [12절](#12-보안--kerberos-인증--ranger-권한)을 한 번 훑어 보세요.
...
[Gateway Node]  ← 여기서 모든 명령 실행 (kinit + security_check + spark-submit)
      │
      │  [보안] Kerberos = 인증(누구인지) · Ranger = 권한(HDFS/HMS/Spark/Ozone 접근)
      │
...
| 4 | `scripts/security/security_check.sh` | 6절 Step 3 — Ranger **권한** probe |
| 5 | `scripts/deployment/package_python.sh` | 6절 Step 4 — zip 빌드 |
| 6 | `scripts/pipeline/run_financial_pipeline.sh` | 7절 — 파이프라인 실행 |
| 7 | `governance/configs/security/ranger.yaml` | Ranger 정책 요청·확인 시 |
| 8 | `governance/configs/environments/dev.yaml` | 경로·리소스 확인 시 |
| 9 | `conf/dev/spark-defaults.conf` | Spark 설정 확인 시 |
...
**security_check.sh란?**  
유효한 Kerberos 티켓이 있는 상태에서 HDFS raw / Ozone Bronze 경로에 **Ranger 정책**으로 접근 가능한지 probe합니다.  
실패 시 `governance/configs/security/ranger.yaml`을 참고해 Ranger 정책을 요청하세요.
...
bash scripts/security/security_check.sh  # Ranger HDFS/Ozone probe
```
...
| **권한 (Authorization)** | **Ranger only** | 정책 인벤토리: `governance/configs/security/ranger.yaml` |

**금지:** `chmod`, `chown`, `setfacl`, `hdfs dfs -chmod`, Ozone native ACL로 접근 제어  
**허용:** Ranger UI/REST API로 HDFS 경로, Ozone volume/bucket, Hive DB/테이블 정책 추가
