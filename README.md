...
# ③ Ozone 준비 — volume/bucket create 는 Ranger Ozone 정책 필요
ozone sh volume list
ozone sh bucket list --volume dev
# chmod/chown 금지 — Ranger 정책으로 systest 에게 create 권한 부여 후:
...
| **Apache Ranger** | HDFS / HMS / Spark / Ozone **권한은 Ranger 정책만** 사용 ([12절](#12-보안--kerberos-인증--ranger-권한)) |
...
1. 플랫폼 팀에 `governance/configs/security/ranger.yaml` 기준으로 **`systest` principal** 정책 요청
2. `bash scripts/security/security_check.sh` 로 HDFS raw / Ozone Bronze 경로 probe
3. `Permission denied` 시 **Ranger UI**에서 정책 추가 — `chmod`/`chown`/`setfacl` **사용 금지**

상세: [12절](#12-보안--kerberos-인증--ranger-권한) · [Ranger 가이드](docs/operations/ranger-authorization.md)
...
# 없으면 생성 — Ranger Ozone 정책으로 systest 에게 volume/bucket create 권한 필요
# chmod/chown/setfacl 로 권한 부여하지 마세요 (Ranger only)
ozone sh volume create dev
ozone sh bucket create --volume dev data
```

Jenkins/CloudCat로 DEV 클러스터를 **재빌드**한 경우 Ozone OM service ID·HMS 호스트가 바뀔 수 있습니다.  
CM 값과 `env.conf`, `governance/configs/environments/dev.yaml`의 `ofs_uri`, `hms_*`를 맞춘 뒤, **Ranger 정책**도 동일 경로에 맞게 갱신하세요 → [Ranger 가이드](docs/operations/ranger-authorization.md).
...
- [ ] Ranger 정책에 `systest` principal에 HDFS raw / Ozone medallion / `sbi_financial` Hive 권한 있음
...
| **Ozone** | volume `dev`, bucket `data` | Medallion 저장소 |
| **Ozone** | `ofs://.../dev/data/{brnz,slvr,gld}` | Bronze/Silver/Gold |
| **Hive** | DB `sbi_financial`, Iceberg 테이블 3개 | HMS / Iceberg catalog |
| **Spark** | (클러스터 설정 시) Hive/HDFS/Ozone과 정렬 | Spark SQL Job |

UAT/PROD는 `environments/{uat,prod}.yaml`의 `medallion` 경로에 맞춰 동일 패턴으로 Ranger 정책을 추가합니다.
...
| **Ranger** | HDFS/HMS/Spark/Ozone **권한(authorization)** 정책 엔진 — SBI는 **Ranger만** 사용 |
| **Delegation Token** | Gateway **인증** 정보를 executor까지 전달 (권한은 Ranger가 결정) |
...
2. **Gateway 설정**
   ...
   bash scripts/security/kinit_manager.sh
   ```
3. **Ranger 정책** — HDFS/HMS/Spark/Ozone 권한은 Ranger만 사용
   - `governance/configs/security/ranger.yaml` 기준으로 `systest` principal 정책 요청
   - `bash scripts/security/security_check.sh` 로 HDFS/Ozone probe
...
pytest tests/ -q --ignore=tests/integration
# 34 tests (환경 설정·Medallion·리소스·Ranger 보안 표준 포함)
```
