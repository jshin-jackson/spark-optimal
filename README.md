...
> Cloudera CDP 7.3.1 · Spark 3.5 · Apache Iceberg · Apache Ozone · Kerberos · **Ranger KMS (Ozone TDE)**
...
| ② | [6절](#6-처음-시작하기--5단계) | clone → env.conf → JAR 검증 → kinit → **security_check(Ranger)** → zip | 15분 |
| ③ | [7절](#7-금융-데이터-파이프라인-실습-예제) | **암호화 bucket** (`ozone_encryption_key`) → 소량 파이프라인 | 10분 |
...
# ② Kerberos(인증) + Ranger probe(권한) + 패키징
bash scripts/security/kinit_manager.sh
bash scripts/security/security_check.sh   # HDFS/Ozone Ranger 정책 probe
bash scripts/deployment/package_python.sh
pip install -r requirements/financial.txt

# ③ Ozone encrypted bucket (Ranger KMS TDE — ozone_encryption_key)
...
> **Kerberos / Ranger / Ozone TDE:** [12절](#12-보안--kerberos-인증--ranger-권한) · [Ozone Encryption](docs/operations/ozone-encryption.md)
...
[Gateway Node]  ← 여기서 모든 명령 실행 (kinit + security_check + spark-submit)
      │
      │  [보안] Kerberos = 인증 · Ranger = 권한 · Ranger KMS = Ozone TDE (ozone_encryption_key)
      │
...
│   └── security/
│       ├── ranger.yaml             ← ★ Ranger 권한 인벤토리 (HDFS/HMS/Spark/Ozone)
...
│   ├── infrastructure/        ← Jenkins/CloudCat, Ozone encrypted bucket
...
| 4 | `scripts/security/security_check.sh` | 6절 Step 3 — Ranger + **KMS key** probe |
| 5 | `scripts/infrastructure/setup_ozone_encrypted_bucket.sh` | 7절 — **`--bucketkey ozone_encryption_key`** |
| 6 | `scripts/deployment/package_python.sh` | 6절 Step 4 — zip 빌드 |
| 7 | `scripts/pipeline/run_financial_pipeline.sh` | 7절 — 파이프라인 실행 |
| 8 | `governance/configs/security/ozone_encryption.yaml` | Ozone TDE 설정·키 이름 |
| 9 | `governance/configs/security/ranger.yaml` | Ranger + KMS ACL 인벤토리 |
| 10 | `governance/configs/environments/dev.yaml` | 경로·리소스 확인 시 |
| 11 | `conf/dev/spark-defaults.conf` | Spark + KMS URI 설정 |
...
| **Ranger KMS + Ozone TDE** | Medallion Ozone 데이터 **전체** `ozone_encryption_key`로 암호화 ([Ozone Encryption](docs/operations/ozone-encryption.md)) |
...
1. 플랫폼 팀에 `governance/configs/security/ranger.yaml` 기준 **`systest` principal** Ranger 정책 요청
2. Ranger KMS **`ozone_encryption_key`** ACL (OM + systest) — [Ozone Encryption](docs/operations/ozone-encryption.md)
3. `bash scripts/infrastructure/setup_ozone_encrypted_bucket.sh` — **`--bucketkey ozone_encryption_key`** bucket
4. `bash scripts/security/security_check.sh` — Kerberos + Ranger + KMS probe

상세: [12절](#12-보안--kerberos-인증--ranger-권한) · [Ranger](docs/operations/ranger-authorization.md) · [Ozone TDE](docs/operations/ozone-encryption.md)
...
**security_check.sh란?**  
유효한 Kerberos 티켓 + HDFS/Ozone **Ranger** probe + Ranger KMS **`ozone_encryption_key`** 존재 여부를 확인합니다.  
실패 시 `ranger.yaml` / `ozone_encryption.yaml`을 참고해 정책·키·암호화 bucket을 점검하세요.
...
`bash scripts/infrastructure/setup_ozone_encrypted_bucket.sh` 로 **암호화 bucket**을 준비한 뒤  
...
### 최초 실행 전: Ozone **암호화** bucket 준비 (Ranger KMS TDE)

**모든 Medallion Ozone 데이터**는 Ranger KMS 키 **`ozone_encryption_key`** 로 **저장 시 암호화(TDE)** 됩니다.

| 항목 | 값 |
|------|-----|
| Ranger KMS service | `cm_kms` |
| Encryption key | **`ozone_encryption_key`** (AES/CTR/NoPadding, 256-bit) |
| Ozone volume | `{SBI_ENV}` (DEV: `dev`) |
| Ozone bucket | `data` (생성 시 `--bucketkey` 필수) |
| Medallion prefix | `ofs://ozone1782570080/{env}/data/brnz\|slvr\|gld/` |

```bash
# 한 번에: volume + KMS 암호화 bucket 생성/검증
export SBI_ENV=dev
source config/env.conf
bash scripts/security/kinit_manager.sh
bash scripts/infrastructure/setup_ozone_encrypted_bucket.sh
...
수동 생성 (암호화 bucket — **키는 생성 시에만** 지정 가능):

```bash
hadoop key list | grep ozone_encryption_key   # Ranger KMS에 키 존재 확인
ozone sh volume create dev                    # Ranger Ozone 정책 필요
ozone sh bucket create --volume dev --bucket data --bucketkey ozone_encryption_key
```

> **주의:** 이미 `--bucketkey` 없이 만든 `data` bucket은 **암호화 불가** — 백업 후 삭제·재생성 필요.  
> **Ranger KMS:** OM 서비스 사용자(Get Metadata, Generate EEK) + `systest`(Generate/Decrypt EEK) 정책 필요.

상세: [Ozone Encryption 가이드](docs/operations/ozone-encryption.md) · [Ranger 가이드](docs/operations/ranger-authorization.md)
...
| `config/env.conf` | Gateway Kerberos, JAR, **`OZONE_ENCRYPTION_KEY`**, **`KMS_PROVIDER_URI`**, Medallion 경로 |
...
bash scripts/security/security_check.sh  # Ranger + KMS probe
bash scripts/infrastructure/setup_ozone_encrypted_bucket.sh  # encrypted bucket (7절)
echo $OZONE_ENCRYPTION_KEY $KMS_PROVIDER_URI
```
...
**허용:** Ranger UI로 HDFS/Ozone/Hive 정책 · Ranger KMS UI로 **`ozone_encryption_key`** ACL · bucket `--bucketkey`

상세: [Ranger Authorization](docs/operations/ranger-authorization.md) · [Ozone Encryption (TDE)](docs/operations/ozone-encryption.md)

### 매 배치 실행 전 체크리스트

- [ ] `bash scripts/security/kinit_manager.sh` 실행 (Kerberos 인증)
- [ ] `bash scripts/security/security_check.sh` 통과 (Kerberos + Ranger 경로 probe)
- [ ] Ranger 정책에 `systest` principal — HDFS raw / Ozone medallion / `sbi_financial` Hive
- [ ] Ozone bucket `/{env}/data` created with **`--bucketkey ozone_encryption_key`**
- [ ] Ranger KMS key ACL for `ozone_encryption_key` (OM + systest)
- [ ] `spark_submit.sh` 사용 (직접 spark-submit 금지 — Delegation Token 누락 위험)
- [ ] cluster deploy mode 사용 (local mode 금지)

### 필수 Ranger 리소스 (DEV 예시)

`governance/configs/security/ranger.yaml`에 전체 목록이 있습니다. 플랫폼 팀에 **`systest@...` principal**로 아래 접근을 Ranger 정책으로 요청하세요.

| Ranger 서비스 | 리소스 | spark-optimal 용도 |
|---------------|--------|-------------------|
| **HDFS** | `hdfs://ns1/dev/raw/financial/transactions` | Raw JSON 업로드 |
| **HDFS** | `hdfs:///user/spark/applicationHistory` | Spark event log |
| **Ozone** | `ofs://.../dev/data/{brnz,slvr,gld}` | Bronze/Silver/Gold (**encrypted bucket**) |
| **Hive** | DB `sbi_financial`, Iceberg 테이블 3개 | HMS / Iceberg catalog |
| **Spark** | (클러스터 설정 시) Hive/HDFS/Ozone과 정렬 | Spark SQL Job |
| **Ranger KMS** | key `ozone_encryption_key` on `cm_kms` | Ozone TDE — OM + systest key ACL |

UAT/PROD는 `environments/{uat,prod}.yaml`의 `medallion`·`ozone_encryption` 경로에 맞춰 동일 패턴으로 정책을 추가합니다.

### Ozone Transparent Data Encryption (TDE)

- KMS key: **`ozone_encryption_key`** (Ranger KMS UI → service `cm_kms`)
- Bucket `/{env}/data` 생성: `ozone sh bucket create ... --bucketkey ozone_encryption_key`
...
volume `dev`, bucket `data`가 없으면 [7절 Ozone 암호화 bucket 준비](#최초-실행-전-ozone-암호화-bucket-준비-ranger-kms-tde)를 먼저 진행하세요.

### Q. Spark/Ozone write fails with KMS / encryption / `GenerateEEK` error

**Ranger KMS 또는 암호화 bucket** 문제입니다.
...
| 확인 | 조치 |
|------|------|
| `hadoop key list`에 키 없음 | Ranger KMS UI (`cm_kms`)에서 `ozone_encryption_key` 생성 |
| bucket이 `--bucketkey` 없이 생성됨 | bucket 재생성: `--bucketkey ozone_encryption_key` |
| OM / systest KMS ACL 없음 | OM: Get Metadata + Generate EEK · systest: Generate/Decrypt EEK |
...
상세: [docs/operations/ranger-authorization.md](docs/operations/ranger-authorization.md)
...
2. **Gateway 설정**
   ...
   bash scripts/security/kinit_manager.sh
   ```
3. **Ranger + KMS** — HDFS/HMS/Spark/Ozone **Ranger 정책** + **`ozone_encryption_key`** KMS ACL
   - `governance/configs/security/ranger.yaml` · `ozone_encryption.yaml`
   - `bash scripts/infrastructure/setup_ozone_encrypted_bucket.sh`
   - `bash scripts/security/security_check.sh`
...
| [Ranger Authorization](docs/operations/ranger-authorization.md) | **Ranger-only** HDFS/HMS/Spark/Ozone 권한 |
...
pytest tests/ -q --ignore=tests/integration
# 36 tests (환경·Medallion·Ranger·Ozone TDE 포함)
```
