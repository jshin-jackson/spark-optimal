...
[Gateway Node]  ← 여기서 모든 명령 실행 (kinit + security_check + spark-submit)
      ...
      │
      ├── 2. HDFS에 Raw 데이터 업로드 (**Encryption Zone**, `/{env}/data/brnz/`)
      │         hdfs://ns1/{SBI_ENV}/data/brnz/transactions
      │
...
HDFS Raw 경로(`/{env}/data/brnz/transactions`)는 Ozone과 동일한 **`/{env}/data`** 레이아웃 아래 **`hdfs_encryption_key`** 로 **Encryption Zone** 암호화됩니다.
...
| 필드 | 값 |
|------|-----|
| Policy Name | `dev-data-brnz` |
| Resource: Path | `hdfs://ns1/dev/data/brnz/transactions` |
| Recursive | **ON** |
...
| DEV HDFS | `/dev/data/brnz/transactions` | `/dev/data/brnz\|slvr\|gld/` (Ozone) |
...
단계 B  HDFS 업로드              →  hdfs://ns1/dev/data/brnz/transactions
...
medallion:
  hdfs_raw_path: hdfs://ns1/dev/data/brnz/transactions
...
echo $HDFS_FINANCIAL_RAW      # .../dev/data/brnz/... 확인
...
- [ ] HDFS Encryption Zone on `/{env}/data/brnz/transactions` (key **`hdfs_encryption_key`**)
...
|---------------|--------|-------------------|
| **cm_hdfs** | `hdfs://ns1/dev/data/brnz/transactions` | Raw JSON 업로드 |
...
- EZ 생성: `hdfs crypto -createZone -keyName hdfs_encryption_key -path /{env}/data/brnz/transactions`
...
# SBI_ENV=dev 이고 .../dev/data/brnz/... 이어야 함
export SBI_ENV=dev && source config/env.conf
```
