...
어려운 용어는 아래 [용어 설명](#용어-설명-초보자용)에서 풀어서 설명합니다.
...
│
├── governance/configs/        ← (상세 구조는 위 참조)
├── docs/                      ← 상세 매뉴얼
│   └── infrastructure/
...
1. `governance/configs/environments/dev.yaml` — DEV 클러스터·경로·리소스 상한
2. `governance/configs/infrastructure/dev_jenkins_build.yaml` — DEV Jenkins/CloudCat 재빌드 파라미터
3. `conf/dev/spark-defaults.conf` — DEV Spark executor/driver 설정
4. `config/env.template.conf` — Gateway Kerberos·JAR·`${SBI_ENV}` 경로
5. `scripts/submit/spark_submit.sh` — Spark Job 실행 래퍼
6. `airflow/dags/sbi_financial_medallion_dag.py` — Airflow 스케줄 정의
...
```bash
# 예: /opt/spark-optimal 에 프로젝트 복사 (경로는 환경에 맞게 조정)
...
```
Step 1  SDV로 JSON 10GB 생성     →  data/output/financial/*.jsonl
Step 2  HDFS 업로드              →  hdfs://ns1/dev/raw/financial/transactions
Step 3  Spark: HDFS → Bronze     →  Iceberg brnz_transactions
Step 4  Spark: Bronze → Gold     →  Iceberg gld_daily_report (리포트용)
```
...
# Step 2: HDFS 업로드 (HDFS_FINANCIAL_RAW 는 env.conf 에서 자동 설정)
bash scripts/data/upload_to_hdfs.sh

# Step 3: Bronze 적재 (경로·리소스는 governance/configs/environments/dev.yaml 기준)
...
2. **Gateway 설정**
   ...
   export SBI_ENV=dev
   cp config/env.template.conf config/env.conf
   # CM에서 HMS/Ozone 주소 변경 시 env.conf · dev.yaml 갱신
   source config/env.conf
   bash scripts/security/kinit_manager.sh
   ```
