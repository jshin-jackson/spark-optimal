...
[Gateway Node]  ← 여기서 모든 명령 실행 (kinit + security_check + spark-submit)
      ...
      │
      ├── 2. HDFS에 Raw 데이터 업로드 (**Encryption Zone**, `/{env}/data/migration/upload`)
      │         hdfs://ns1/{SBI_ENV}/data/migration/upload
      │
...
HDFS Raw 경로(`/{env}/data/migration/upload`)는 **`/{env}/data`** 아래 migration upload 영역이며 **`hdfs_encryption_key`** 로 **Encryption Zone** 암호화됩니다.  
Ozone Medallion은 `/{env}/data/brnz|slvr|gld/` (별도 경로).

예) DEV HDFS: `.../dev/data/migration/upload` · DEV Ozone Bronze: `.../dev/data/brnz/`
...
│
├── conf/
│   ├── dev/spark-defaults.conf   ← DEV Spark 기본값 (72 vCore 클러스터용)
...
| 필드 | 값 |
|------|-----|
| Policy Name | `dev-data-migration-upload` |
| Resource: Path | `hdfs://ns1/dev/data/migration/upload` |
| Recursive | **ON** |
...
| DEV HDFS | `/dev/data/migration/upload` | `/dev/data/brnz\|slvr\|gld/` (Ozone) |
...
단계 B  HDFS 업로드              →  hdfs://ns1/dev/data/migration/upload
...
| 총 vCore | 72 |
| 총 Memory | 288 GB |
| NodeManager | 9대 (각 16 vCore, 32 GiB) |
| executor cores | 2 |
...
resource_limits:
  max_executors_absolute: 24
  shuffle_partitions: 48

medallion:
  hdfs_raw_path: hdfs://ns1/dev/data/migration/upload
...
echo $HDFS_FINANCIAL_RAW      # .../dev/data/migration/upload 확인
...
- [ ] HDFS Encryption Zone on `/{env}/data/migration/upload` (key **`hdfs_encryption_key`**)
...
|---------------|--------|-------------------|
| **cm_hdfs** | `hdfs://ns1/dev/data/migration/upload` | Raw JSON 업로드 |
...
- EZ 생성: `hdfs crypto -createZone -keyName hdfs_encryption_key -path /{env}/data/migration/upload`
...
|------|-------|--------|-------------|-----------|
| DEV | 72 | 288 GB | 9대 (16 vCore, 32 GiB) | `governance/configs/environments/dev.yaml` |
...
# SBI_ENV=dev 이고 .../dev/data/migration/upload 이어야 함
export SBI_ENV=dev && source config/env.conf
```
